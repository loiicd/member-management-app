...
import { checkIfLoginEmailExists, registerUser } from '../services/authenticate'
...
const RegisterUserPage: FunctionComponent = () => {
  ...
  const handleSubmit = () => {
    ...
    if (!email) {
      return setErrors({ ...errors, email: 'Bitte E-Mail eingeben' })
    }

    if (!password) {
      return setErrors({ ...errors, password: 'Bitte Passwort eingeben' })
    }
    ...
    if (!firstname) {
      return setErrors({ ...errors, firstname: 'Bitte Vorname eingeben' })
    }

    if (!lastname) {
      return setErrors({ ...errors, lastname: 'Bitte Nachname eingeben' })
    }

    if (password !== passwordConfirmation) {
      return setErrors({ ...errors, password: 'Passwörter sind nicht identisch' })
    }
            ...
            <div className="mt-8 grid grid-cols-6 gap-6">
              ...
              <div className="col-span-6">
                <p className="text-sm text-gray-500">
                  By creating an account, you agree to our <button className='text-gray-700 hover:underline cursor-pointer'>terms and conditions</button> and <button className='text-gray-700 hover:underline cursor-pointer'>privacy policy</button>.
                </p>
              </div>
