...
export class QualificationEntityService {
  ...
  async updateQualification(client: Client, qualification: QualificationType): Promise<void> {
    const query = 'UPDATE public."qualification" SET name = $1, abbreviation = $2, color = $3 WHERE id = $4'
    const values = [qualification.name, qualification.abbreviation, qualification.color, qualification.id]
    await client.query(query, values)
  }
